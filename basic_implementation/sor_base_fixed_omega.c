...
int main(void)
{
    ...
    // создаём папку для вывода (если не существует – системная ошибка игнорируется)
    int make_fold = system("mkdir -p RESULT");
    (void)make_fold;
         ...
         double *tot_time, int *iter, int BC)
{
    ...
    for (it = 1; it <= ITMAX; ++it) {
        ...
        // ------------------------
        if (BC == 1) {
            // Case 1: u=0 сверху/снизу, Neumann по x слева/справа
            int jj;
            for (jj = 0; jj < COL; ++jj) {
                p_new[0][jj]      = 0.0;
                p_new[ROW-1][jj]  = 0.0;
            ...
            // Case 2: аналитическое решение на границе
            int jj;
            for (jj = 0; jj < COL; ++jj) {
